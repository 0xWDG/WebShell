...
@objc extension WSViewController {
    ...
    func injectWebhooks(_ jsContext: JSContext!) {
        ...
        // Issue: #5
        if (settings?.openInNewScreen ?? true) {
            // _blank to external
        ...
        if (settings?.consoleSupport ?? true) {
        ...
        let windowOpen: @convention(block) (NSString?, NSString?, NSString?, NSString?) -> Void = { (url: NSString?, _: NSString?, specs: NSString?, _: NSString?) in
    ...
    /**
     ...
     - Note: @wdg #25
     */
    func resetLocalStorage(_ Sender: AnyObject?) {
    ...
    /**
     ...
     - Note: @wdg #25
     */
    func parseWindowOpen(_ url: String, options: String) {
